	...
	void FixedUpdate()
	{
		...
		if (health > 0)
		{
			if (automatic && vp.groundedWheels > 0)
			{
				...
				targetRatio = (1 - vp.burnout) * Mathf.Clamp01(Mathf.Abs(targetDrive.feedbackRPM) / maxRPM);
			}
			else if (!automatic)
			{
		...
		newDrive.torque = Mathf.Abs(currentRatio) * targetDrive.torque * (legacyTorqueMultiplier ? newDrive.curve.Evaluate(targetDrive.feedbackRPM * 0.001f * Mathf.Abs(currentRatio)) : 1);
		SetOutputDrives(1);
	}
}