...
public class GasMotor : Motor
{
	...
	public override void FixedUpdate()
	{
		...
		{
			//Set RPM
			targetDrive.rpm = Mathf.Lerp(targetDrive.rpm, actualInput * maxRPM * 1000 * (boosting ? 1 + boostPowerCurve.Evaluate(Mathf.Abs(vp.localVelocity.z)) : 1), (1 - inertia) * Time.timeScale);
			//Set torque
			...
			{
				targetDrive.torque = torqueCurve.Evaluate(targetDrive.feedbackRPM * 0.001f) * Mathf.Lerp(targetDrive.torque, power * Mathf.Abs(System.Math.Sign(actualInput)), (1 - inertia) * Time.timeScale) * (boosting ? 1 + boostPowerCurve.Evaluate(Mathf.Abs(vp.localVelocity.z)) : 1) * health;
			}
