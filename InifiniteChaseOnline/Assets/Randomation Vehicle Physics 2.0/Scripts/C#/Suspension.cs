...
public class Suspension : MonoBehaviour
{
	...
	public AnimationCurve camberCurve = AnimationCurve.Linear(0, -10, 1, 10);
	[System.NonSerialized]
	...
	void ApplySuspensionForce()
	{
		if (wheel.grounded && wheel.connected)
		{
			...
			{
				rb.AddForceAtPosition(
					-vp.norm.TransformDirection(0, 0, Mathf.Clamp(travelVel, -hardContactSensitivity, 0) + penetration) * hardContactForce
					, applyForceAtGroundContact ? wheel.contactPoint.point : wheel.tr.position
					, ForceMode.Acceleration);
			}
		}
	...
	void GetCamber()
	{
		...
		{
			camberAngle = camberCurve.Evaluate((Application.isPlaying && wheel.connected ? wheel.travelDist : targetCompression));
		}
	}
