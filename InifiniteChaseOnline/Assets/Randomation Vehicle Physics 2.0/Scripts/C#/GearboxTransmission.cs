...
public class GearboxTransmission : Transmission
{
	...
	public bool autoCalculateRpmRanges;
	...
	void FixedUpdate()
	{
		...
		shiftTime = Mathf.Max(0, shiftTime - Time.timeScale);
		...
		else
		{
			...
			newDrive.torque = Mathf.Abs(curGearRatio) * targetDrive.torque * (legacyTorqueMultiplier ? newDrive.curve.Evaluate(actualFeedbackRPM * 0.001f) : 1);
		}
